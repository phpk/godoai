...
import { getSystemKey } from "@/stores/config";
interface Props {
  content: string;
  role?: string;
  link?: any;
...
const imgList = computed<any[]>(() => {
  if (props.link && props.link.length > 0) {
    const list: any = [];
    const url = getSystemKey("knowledgeUrl") + "/showimage?path=";
...
async function showDetail(file: string, type: string) {
  const filePath = getSystemKey("knowledgeUrl") + "/filedetail?path=" + file
  if(type != "image") {
  ...
  <div>
    ...
    <div v-if="props.role === 'assistant'" class="assistant align-start spacing">
      ...
      <div class="message assistant-message rounded-al">
        <div v-html="renderMarkdown(props.content, false)"></div>
